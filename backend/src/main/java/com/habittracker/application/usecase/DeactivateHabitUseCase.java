...
public class DeactivateHabitUseCase {

    private final HabitRepository habitRepository;
    private final HabitApplicationMapper mapper;

    public HabitResponse execute(DeactivateHabitCommand command) {
        log.info("Desativando hábito: {} para usuário: {}", command.habitId(), command.userId());
        ...
        log.info("Habito desativado com sucesso: {} pelo usuário {}", habitId, userId);
    ...
    private void validateHabitOwnership(Habit habit, UserId requestingUser) {
        if (!habit.getUserId().equals(requestingUser)) {
            throw new ApplicationException("Usuário não tem permissão para alterar esse hábito.");
        }
    }
}
